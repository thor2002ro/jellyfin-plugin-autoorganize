...
def validate_archive(contents, asset_name):
    try:
        with zipfile.ZipFile(io.BytesIO(contents)) as archive:
            ...
            required_files = {"AutoOrganize.dll", "Lingua.dll"}
            missing = required_files.difference(files)
            if missing:
                raise ValueError(f"{asset_name} is missing {', '.join(sorted(missing))}")
            if not any(path.startswith("Lingua/LanguageModels/") for path in files):
                raise ValueError(f"{asset_name} is missing Lingua language models")
            unexpected = [
                path
                for path in files
                if path not in required_files and not path.startswith("Lingua/LanguageModels/")
            ]
            if unexpected:
                raise ValueError(f"{asset_name} contains unexpected files: {', '.join(unexpected)}")
            if any(archive.getinfo(path).file_size == 0 for path in files):
                raise ValueError(f"{asset_name} contains an empty file")
